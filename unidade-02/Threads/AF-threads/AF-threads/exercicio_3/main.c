...
void *worker_thread(void *strct_elementos)
{
    ...
    elementos->resultado[elementos->index] = 0;

    for (size_t i = elementos->loop_size_min; i < elementos->loop_size_max; i++)
        elementos->resultado[elementos->index] += elementos->vetor_a[i] * elementos->vetor_b[i];

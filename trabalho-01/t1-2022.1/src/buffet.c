...
{   
    ...
    while (all_students_entered == FALSE)
    {
        ...
        /* Verifica se o a variavel que indica os estudantes que ja terminaram de comer é igual ao numero de estudantes que entraram no RU */
        if(globals_get_finished_students() == globals_get_students()) all_students_entered = TRUE;
    }

    printf("Acabou; %d", globals_get_finished_students());
    _log_buffet(self);
    pthread_exit(NULL);
}
...
void buffet_init(buffet_t *self, int number_of_buffets)
{
    ...
    for (i = 0; i < number_of_buffets; i++)
    {
        ...
        for(j = 0; j < 5; j++) {
            /* Inicia um semaforo com valor 40 para cada bacia de comida */
            sem_init(&self[i]._sem_meal[j], 0, 40);
            self[i]._meal[j] = 40;
        }
